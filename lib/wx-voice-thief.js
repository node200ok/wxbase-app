	...
	getMsgList(function(err, msgList) {
        ...
        if (! lastMediaId) {
            ...
            if (index === -1) {
                ...
                callback(new Error('Not Yet Media'));
                return;
            } else {
        ...
        lastMediaId = mediaId;
        client.get('/cgi-bin/getvoicedata?msgid=' + mediaId + '&fileid=&token=' + token + '&lang=zh_CN',
        {}, {}, function(err, res, buf) {
            var media = _.findWhere(mediaList, {
                'id': mediaId
            });
            media['data'] = buf;
            callback(null, media);
